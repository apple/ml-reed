...
from stable_baselines3.common.monitor import Monitor, MetaWorldMonitor
...
def make_vec_dmcontrol_env(
    ...
    def make_env(rank):
        def _init():
            ...
            env = utils.ppo_make_env(env_id, seed + rank)
...
def make_vec_metaworld_env(
    ...
    def make_env(rank):
        def _init():            
            env = utils.ppo_make_metaworld_env(env_id, seed + rank)
            if seed is not None:
                env.seed(seed + rank)
                env.action_space.seed(seed + rank)
            # Wrap the env in a Monitor wrapper
